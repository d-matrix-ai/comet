...
# def generate_factor_sets(num, N, constraints):
#     # Generate all factors of the number
#     factors = [i for i in range(1, num + 1) if num % i == 0]
    
#     # Generate all possible sets of size N allowing repetition --> by repetition it means that the same factor can be used multiple times in a set. (1,1,24) is different from (1,24,1)
#     factor_sets = list(product(factors, repeat=N))
    
#     # Filter sets based on constraints
#     valid_sets = [
#         f_set for f_set in factor_sets
#         if check_constraints(f_set, num, constraints)
#     ]
...
def run(data, id, config, output, dimension_dict):
    ...
    default_values = {
        ...
        # Shared Memory
        'M_SM': 1, #data['M'][7],               # 7
        'N_SM': data['N'][7], # * data['N'][7] if (data['N'][6] * data['N'][7]) < 16 else 16, #16 if(data['N'][6]*data['N'][7])>16): data['N'][6]*data['N'][7], #data['N'][7],      simd width         # 7
    ...
    os.system(f'../../build/comet --constants_file {config}/{filename} --arch_file arch.yaml --problem_file problem.yaml --mapping_file mapping.yaml --calc_noc_energy 2>&1 | tee {output}/{output_filename}') #2>&1 redirects stderr (file descriptor 2) to stdout (file descriptor 1), so both are captured by tee.
...
# print(f"Total number of factor sets: {len(factor_sets)}")
if __name__ == "__main__":
    ...
    # os.system('mkdir - args.config_folder_name')
    #if folder doesn't exist then only do mkdir
    current_dir_name = os.path.dirname(os.path.abspath(__file__))
    # config_folder_path = os.path.join(current_dir_name, args.config_folder_name + "_" + dim_str)
    # output_folder_path = os.path.join(current_dir_name, args.output_folder_name + "_" + dim_str)

    config_folder_path = os.path.join(current_dir_name, "simulation_edge" + "_" + dim_str, args.config_folder_name)
    output_folder_path = os.path.join(current_dir_name, "simulation_edge" + "_" + dim_str, args.output_folder_name)

    if not os.path.exists(config_folder_path):
        folder_name=args.config_folder_name + dim_str
        # os.system(f'mkdir {args.config_folder_name + "_"+dim_str}')
        # os.system(f'mkdir {config_folder_path}')
        # os.makedirs()
        os.makedirs(config_folder_path, exist_ok=True)
    # else:
    #     print(f'{config_folder_path} already exists')
    #     sys.exit(1)

    if not os.path.exists(output_folder_path):
        # os.system(f'mkdir {args.output_folder_name + "_"+dim_str}')
        # os.system(f'mkdir {output_folder_path}')
        os.makedirs(output_folder_path, exist_ok=True)

    # else:
    #     print(f'{output_folder_path} already exists')
    #     sys.exit(1)
