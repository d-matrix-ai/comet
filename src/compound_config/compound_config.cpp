...
bool CompoundConfigNode::isArray() const {
  ...
  else if (YNode) return YNode.IsSequence() && YNode[0].IsScalar(); //isSeq checks if YNode is an array or list
  else {
    assert(false);
    return false;
  }
...
CompoundConfig::CompoundConfig(std::vector<std::string> inputFiles) {
  ...
  if (std::strstr(inputFiles[0].c_str(), ".cfg")) {
    ...
    try {
        ...
        // Extract line number and column from exception message
        ...
        if (linePos != std::string::npos && columnPos != std::string::npos) {
            int errorLine = std::stoi(errorMsg.substr(linePos + 5, columnPos - (linePos + 5)));

            // Store all lines of combinedString for easier access
            std::istringstream stream(combinedString);
            ...
            if (errorLine > 1) {  // Print the previous line if it exists
                std::cerr << "Line " << errorLine - 1 << ": " << lines[errorLine - 2] << std::endl;
            }
            std::cerr << "Line " << errorLine << " (Error): " << lines[errorLine - 1] << std::endl;
            if (errorLine < static_cast<int>(lines.size())) {  // Print the next line if it exists
                std::cerr << "Line " << errorLine + 1 << ": " << lines[errorLine] << std::endl;
            }
        }
        exit(EXIT_FAILURE);
    }
