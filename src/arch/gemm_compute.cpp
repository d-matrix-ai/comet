#include "arch/gemm_compute.hpp"


namespace arch {
  REGISTER_CLASS(GEMMCompute, ComputeBase);  
  uint64_t GEMMCompute::getComputeLatency(problem::DimSizeExpression tilesize, mapping::computation_attributes comp_attr, mapping::operation_t op_type) {
  ...
  uint64_t GEMMCompute::getIdealSpecLatency(problem::DimSizeExpression tilesize) { 
    if (tilesize.size() == 3) { // should this be based on computation_attribute type
  ...
  void GEMMCompute::setAttributes(YAML::Node& YNode) {
    node = YAML::Clone(YNode);
  }

  float GEMMCompute::getComputeEnergy(problem::DimSizeExpression tilesize, mapping::computation_attributes comp_attr, mapping::operation_t op_type){
    return 0;
  }
